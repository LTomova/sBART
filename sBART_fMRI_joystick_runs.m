...
function sBART_fMRI_joystick_runs
...
% 24 trials per cond, 1 run, 12 trials long (approx. 6min)
% this task is same as sBART_fMRI_joystick but runs only 1 run at once
% command to start sBART: sBART_fMRI_joystick_runs;
...
for run = 1:1
    %create cond order (random permutations of 1,2,3)
    condpm=randperm(3);
    ...
    text1 =[ 'Individual round. Run ', num2str(runNr),' of 6'];
    ...
    for trialNum = 1:12
        ...
        if confirm ~= 1
            ...
            %log trialinformation already here if response missed
        log(trialNum,1)=9999;
        log(trialNum,2)=9999;
        log(trialNum,3)=9999;
        log(trialNum,4)=9999;
        ...
        onsets(trialNum,2)=triggerTime;
        onsets(trialNum,3)=trialStart;
        onsets(trialNum,4)=Response1Start;
        onsets(trialNum,5)=9999;
        onsets(trialNum,6)=9999;
        onsets(trialNum,7)=9999;
        onsets(trialNum,8)=9999;
        onsets(trialNum,9)=9999;
        ...
        duration_other(trialNum)=9999;
        duration_resp2(trialNum)=9999;
        duration_choice(trialNum)=9999;
        duration_balloon(trialNum)=9999;
        duration_outcome(trialNum)=9999;
        ...
        onsets(trialNum,3)=trialStart;
        onsets(trialNum,4)=Response1Start;
        onsets(trialNum,5)=OtherInfoStart;
        onsets(trialNum,6)=Response2Start;
        onsets(trialNum,7)=9999;
        onsets(trialNum,8)=9999;
        onsets(trialNum,9)=9999;
...
%         SoundOut(1,inflate); %sound
        ...
        onsets_names={'experimentStart','triggerTime','trialStart', 'Response1Start', 'OtherInfoStart', 'Response2Start', 'ChoiceInfoStart', 'BalloonStart','OutcomeStart'};
        ...
        onsets(trialNum,5)=OtherInfoStart;
        onsets(trialNum,6)=Response2Start;
        onsets(trialNum,7)=ChoiceInfoStart;
        onsets(trialNum,8)=BalloonStart;
        onsets(trialNum,9)=OutcomeStart;
    ...
    Screen('Flip',window);
        if runNr ==6
            ...
            wait_rundur=add_fix(1,runNr);
            WaitDur=10+wait_rundur;
            WaitSecs(WaitDur);
        end
    Screen('Close');
    RunEnd=GetSecs;
    ...
    if runNr == 6
        ...
        Screen('Flip',window);
        cd(subjectdir);
        save([subID '.sBART_fmri_run', num2str(runNr),'.mat']);
